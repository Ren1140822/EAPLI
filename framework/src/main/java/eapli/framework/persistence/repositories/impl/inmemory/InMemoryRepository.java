...
public class InMemoryRepository<T extends DomainEntity<K>, K extends Serializable>
	...
	/**
	 * This method is used for searching a list without using Optional and Streams, thus returning null when no element is found.
	 *
	...
	/**
	 * This method is only used for showing the usage of Optional and streams to avoid returning null. In either case, the client code must check for NoSuchElementException.
	 *
	...
	/*@Override
	public T findById(K id) {
        Optional<T> value = repository.stream().filter(a -> a.id().equals(id)).findFirst();
        T t= value.get();
        return t;
    }*/
	@Override
